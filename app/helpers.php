...
function H_JWT_decode($token){
    try {
        ...
        return H_apiResponse(null, 'Token expired', 401);
    }
...
function H_findObjectByKey2($array, $key, $value, $getIndex = false)
{
    for ($i=0; $i < count($array) ; $i++) { 
        if ($array[$i]->{$key} === $value) {
            if ($getIndex) return $i;
...
function H_dayBetweenDate($start_date, $end_date) {
    return Carbon::parse( $start_date )->diffInDays( $end_date );
}

function H_getGanttDuration($day) {
    return $day * 24 * 60 * 60 * 1000;
}

function H_sumArrayObjectBy($raw, $column, $column2 = null) {   
