...
trait StandardRepo {
  ...
  */
  public function searchable($data, $payload, $primaryTable = null, $inside = false) {
    try {
      if (H_hasRequest($payload, 'trash')) $data = $data->onlyTrashed();
          ...
          if ($this->validateColumns($params['key'])) {
            ...
            if ($relField) { // relation search
              ...
              if ($operator) {
                $value = H_getValueForOperator($key, $value);
                if ($inside) {
                  $data = $data->where(function ($q) use ($column, $operator, $value) {
                    $q->whereRaw("$column $operator ?", [$value]);
                  });
                } else $data = $data->whereRaw("$column $operator ?", [$value]);
                ...
                if (!$inside) {
                  ...
                  if ($value == 'not_null') {
                    $data = $data->where(function ($q) use ($column) {
                    ...
                    $data = $data->where(function ($q) use ($column, $value) {
                      $q->whereRaw(''.$column.' = ? ', [$value]); // jika menggunakan [!]
                    });
                  } else {
                    $data = $data->where(function ($q) use ($column, $value) {
                      $q->whereRaw(''.$column.' = ? OR '.$column.' like ?', [$value,'%'.$value.'%']);
                    });
                  }
                }
              }
