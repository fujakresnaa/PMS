      ...
      <div class="q-pa-md">
        ...
        >
          ...
          <template v-slot:body-cell-action="props">
            <q-td :props="props" >
              <q-btn v-if="credentials !== null && props.row.id === credentials.id" class="bg-grey-2" dense round flat color="green" @click="edit(props.row)" icon="edit"></q-btn>
              <q-btn v-if="credentials !== null && props.row.id === credentials.id" class="bg-grey-2" dense round flat color="primary" @click="detail(props.row)" icon="visibility"></q-btn>
            </q-td>
          </template>

          <template v-slot:body-cell-role="props">
            <q-td :props="props">
              <span v-if="props.row.role" >
              <q-badge color="primary">{{props.row.role_id}}</q-badge>
              {{props.row.role.name}}
              </span>
            </q-td>
          </template>
          ...
          <template v-slot:body-cell-menu="props">
            <q-td :props="props">
              ...
              <q-badge color="primary">{{props.row.menu_id}}</q-badge>
              {{props.row.menu.name}}
              </span>
            </q-td>
          </template>
          ...
          <template v-slot:body-cell-active="props">
            <q-td :props="props">
              <q-btn unelevated size="xs"
              :color="(props.row.active === 0) ? 'red' : 'green' "
              :label="(props.row.active === 0) ? 'UnActive [0]' : 'Active [1]' "
              />
            </q-td>
          </template>
          ...
          <template v-slot:top>
            ...
            <div v-if="rules.permission.delete && table.selected.length !== 0 ">
              <q-btn v-if="false" @click="deleteSelected" unelevated class="capital "
  ...
  methods: {
    ...
    initTopBar () {
      this.Meta.topBarMenu = [
        { name: 'Dashboard', event: this.toDashboard },
