<template>
  <div>
    <q-header class="bg-grad" >
      <q-toolbar>
        <q-btn v-if="!disabledDrawer" flat @click="drawerCtrl" round dense icon="menu" />
          ...
          >
            <div class="no-wrap q-pa-md">
              ...
              <div class="">
                ...
                <q-btn @click="$router.push({name:'update-profile-users'})" flat color="primary" size="sm" class="bg-grey-2 capital mb-1 full-width" icon="person" label="Profile" /> <br>
                <q-btn @click="$router.push({name:'change-password-users'})" flat color="primary" size="sm" class="bg-grey-2 capital mb-1 full-width" icon="gpp_good" label="Change Password" /> <br>
    ...
    <q-drawer v-if="!disabledDrawer" bordered @before-show="activate" class="bg-dark" :behavior="menuBehavior"
      ...
      :breakpoint="500"
    >
      <q-scroll-area class="drawer-area" style="height: calc(100% - 80px); margin-top: 70px;">
        <q-list v-for="(menuItem, index) in menuList" :key="index">

          <q-item v-if="menuItem.overline"  class="animated fadeIn single-line-menu" >
          ...
          <q-expansion-item v-if="menuItem.sub.length !== 0"
            ...
            <q-list v-for="(menuSubItem, index) in menuItem.sub" :key="index">
              ...
              <q-item  class="animated fadeIn single-line-menu" v-if="menuSubItem.sub.length === 0" clickable @click="actionMenu(menuSubItem)" v-ripple>
          ...
          <q-item  class="animated fadeIn single-line-menu" v-if="menuItem.sub.length === 0" clickable @click="actionMenu(menuItem)" v-ripple>
            <q-item-section avatar> <q-icon style="font-size: 1.1em;"  :name="menuIcon(menuItem.detail.icon)" /> </q-item-section>
            <q-item-section class="">{{ menuItem.detail.name }} </q-item-section>
          </q-item>

        </q-list>
      </q-scroll-area>

      <q-img @click="toRoot" class="absolute-top animated fade-in" style="height: 80px; !important;">
        <q-list class="bg-transparent" style="width: 100%;padding: 0px 0px 0px 0px;">
          <q-item class="ph pv bg-soft">
            <div class="text-center" style="width: 100%;">

             <q-item v-if="!miniState" clickable v-ripple class="ph pv">
                <q-item-section side>
                  <q-avatar rounded size="52px"> <img src="assets/icon.png"/> </q-avatar>
                </q-item-section>
                <q-item-section class="left" style="width: 100%;" >
                  <q-item-label v-if="project" ><b class="text-primary">{{project.code}}</b></q-item-label>
                  <q-item-label v-if="project" class="text-dark"><small>{{project.name}}</small></q-item-label>
                </q-item-section>
              </q-item>

              <img v-if="miniState" class="animated fadeIn" src="assets/icon.png" width="52">
            </div>
          </q-item>
...
export default {
  name: 'Drawer',
  props: ['topBarMenu', 'topBarInfo', 'isOpenDrawer', 'display', 'reload', 'disableDrawer'],
  data () {
    return {
      moduleName: 'Drawer',
  ...
  created () {
    ...
    this.project = this.$Helper.getLdb('project')
    // console.log('project', this.project)
  },

  mounted () {
    if (this.disableDrawer) this.disabledDrawer = this.disableDrawer
    ...
    if (credentials !== false) {
      ...
      if (route === 'timeline') this.drawer = false // khusus timeline
    }
  },
  ...
  methods: {
    ...
    getNotif () {
      ...
      this.$Helper.loading()
      this.API.get('me/notifications?limit=0', (status, data, message, response, full) => {
    ...
    toRoot () {
      var data = { name: 'view-projects', params: this.project }
      this.$router.push(data)
    }
  }
}
