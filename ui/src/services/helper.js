...
export const Helper = {
  ...
  getLdb (key) {
    ...
    if (!this.checkLdb(key)) {
      console.info('LDB [' + key + '] Not found!')
      return null
    } else return data
  ...
  getKeyName (data, index) {
    var label = this.createLabelFromKey(data)
    return label[index]
  },
  ...
  getFirstChar (str, additional = null) {
    if (additional) {
      return str.charAt(0) + str.charAt(additional)
  ...
  firstOfMonth () {
    var date = this.toDate(this.getDateNow(), 'YYYY-MM-') + '01'
    return date
  },
  ...
  async filterSelect (val, update, target, selectSource) {
    ...
    await update(() => {
      ...
      selectSource[target] = tmp.filter(v => v.name.toLowerCase().indexOf(needle) > -1)
    })
    return selectSource
  },
  ...
  logInfo (date) {
    const tanggal = new Date(date),
      ...
      jam = tanggal.getHours(),
      menit = tanggal.getMinutes()
    const sbulan = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember']
    // const sbulan = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']

    const res = `${tgl} ${sbulan[bln]} ${thn} - ${jam}:${menit}`
    return res
  },
  ...
  goToPage (_this, name, params = null) {
  ...
  hasProjectId (_this) {
    ...
    // if (_this.$Helper.checkParams(params)) {
    //   if (params.project_id !== undefined && params.project_id !== null) {
    //     res = true
    //   }
    // }

    var project = this.getLdb('project')
    // console.log('hasProjectId', project)

    if (!project) _this.$router.push({ name: 'home' })
    else return project.id
  },

  emitModel (target, val, model) {
    model[target] = val
    return model
  }
}
